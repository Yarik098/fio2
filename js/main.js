;(() => {
	var e = {
			717: () => {
				const e = document.getElementById('dropdownBtn'),
					t = document.querySelectorAll('.js-dropdown-item')
				e.addEventListener('click', function () {
					e.classList.toggle('js-dropdown-btn'),
						t.forEach(function (e) {
							e.classList.toggle('js-dropdown-item')
						})
				})
			},
			681: () => {
				const e = document.querySelectorAll('input[type="radio"]')
				e.forEach(t => {
...
document.addEventListener('DOMContentLoaded', function () {
	let blocks = document.querySelectorAll('.modal__item')

	blocks.forEach(function (block) {
		block.addEventListener('click', function () {
			blocks.forEach(function (b) {
				b.classList.remove('js-modal-item')
			})
			block.classList.add('js-modal-item')
		})
	})
})

document.addEventListener('DOMContentLoaded', function () {
	const deleteButtons = document.querySelectorAll('.table__btn')

	deleteButtons.forEach(button => {
		button.addEventListener('click', function () {
			const row = this.closest('.table__row')
			const idElement = row.querySelector('.table__col .table__text')

			if (idElement) {
				const id = idElement.innerText

				fetch('/vendor/delete.php', {
					method: 'POST',
					headers: {
